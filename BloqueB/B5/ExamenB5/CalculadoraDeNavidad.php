...
class CalculadoraDeNavidad
{
    ...
    public function repartirRegalos(array $asignaciónDeRegalos, ListaDeDeseos $deseos): array
    {
        ...
        // Recorremos cada casa y su cantidad de regalos asignados
        foreach ($asignaciónDeRegalos as $indice => $numRegalos) {
            $carta = $deseos->obtenerCartas()[$indice] ?? ""; // Obtener la carta asociada

            if (strpos($carta, "***") !== false) { // Palabra prohibida detectada
                $resultado[$indice] = ["carbón"];
            } else {
                $resultado[$indice] = [];
                for ($j = 0; $j < $numRegalos; $j++) {
                    $resultado[$indice][] = $deseos->obtenerRegalo(); // Obtener un regalo
                }
            }
        }
    ...
    public function calcularCosteRegalos(array $regalos, ListaDeDeseos $listaDeseos): float
    {
        ...
        foreach ($regalos as $regalo) {
            $coste += $listaDeseos->obtenerPrecioRegalo($regalo);
        }
    ...
    public function mostrarEstadísticas(array $repartoRegalos, array $analisis): void
    {
        ...
        foreach ($repartoRegalos as $regalosCasa) {
            $totalRegalos += count($regalosCasa);
        }
        ...
        // Mostrar estadísticas en formato HTML
        ...
        echo "<li>Número de aciertos: $aciertos</li>";
        echo "<li>Porcentaje de aciertos: " . number_format($porcentajeAciertos, 2) . "%</li>";
        echo "</ul>";

        if ($porcentajeAciertos >= 50) {
            echo "<p>¡La Navidad fue feliz!</p>";
        } else {
            echo "<p>La Navidad fue triste...</p>";
        }
    }
}
