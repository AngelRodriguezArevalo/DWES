...
class ListaDeDeseos
{
    ...
    public function __construct($palabrasProhibidas = [], $posiblesRegalos = ["bicicleta" => 299.99, "patinete" => 499.90, "videoconsola" => 599.99])
    {
        ...
        // Generar el doble de regalos disponibles
        for ($i = 0; $i < count($this->posiblesRegalos) * 2; $i++) {
            $regalo = array_rand($this->posiblesRegalos);
            array_push($this->regalosDisponibles, $regalo);
        }
    }
    ...
    public function obtenerPrecioRegalo(string $nombreRegalo): float
    {
        ...
        return $this->posiblesRegalos[$nombreRegalo] ?? 0.0;
    }
    ...
    public function agregarCarta(string $texto)
    {
        ...
        $textoLimpio = $this->limpiarPalabrasProhibidas($textoLimpio);
        $this->cartas[] = $textoLimpio;
    }
    ...
    {
        $resultados = [];
        foreach ($repartoDeRegalos as $indice => $regalosCasa) {
            $carta = $this->cartas[$indice] ?? "";
            $cumplido = false;

            foreach ($regalosCasa as $regalo) {             
                $regex = "/\b" . $regalo . "\b/u";
                if (preg_match($regex, $carta)) {
                    $cumplido = true;
                    break;
                }
            }
